...
// Collapsible component for long descriptions
...
export default function ShopTab() {
    ...
    const [activeScenarioText, setActiveScenarioText] = useState("正在四處張望...");
    ...
    useEffect(() => {
        const updateActiveToken = async (selection: string[] | undefined | null) => {
            ...
            if (safeSelection.length !== 1) {
                setActiveTokenId(null); setActiveTokenName("請點選角色"); setActiveTokenGold(null); setActiveTokenWeight(null); return;
            }
        ...
        const handleShopResponses = async (message: any) => {
            ...
            if (data.type === "PURCHASE_RESPONSE" || data.type === "SELL_RESPONSE") {
                if (data.success) {
                    ...
                    setToastMsg({ text: data.type === "SELL_RESPONSE" ? `出售成功！獲得 ${(data.revenue * data.quantity).toFixed(2)} GP。` : data.message, type: "success" });
                    if (activeTokenId) fetchTokenData(activeTokenId);
                } else { setToastMsg({ text: data.message, type: "error" }); }
    ...
    const handleBuy = async (shop: Shop, item: ShopItem, quantity: number, isBoughtItem: boolean = false) => {
        ...
        if (!activeTokenId) { setToastMsg({ text: "請先點選你要購物的角色！", type: "error" }); setTimeout(() => setToastMsg(null), 3000); return; }
    ...
    const getMultiplierIcon = (mult: number) => {
        if (mult <= 0.5) return <span className="text-yellow-400 flex items-center gap-1 text-[18px] ml-2"><FaCrown /></span>;
        if (mult < 1.0) return <span className="text-emerald-400 flex items-center gap-1 text-[18px] ml-2"><FaFaceSmile /></span>;
        if (mult === 1.0) return <span className="text-gray-300 flex items-center gap-1 text-[18px] ml-2"><FaFaceMeh /></span>;
        if (mult <= 1.5) return <span className="text-orange-400 flex items-center gap-1 text-[18px] ml-2"><FaFaceAngry /></span>;
        return <span className="text-red-400 flex items-center gap-1 text-[18px] ml-2"><FaSkull /></span>;
    };
                                            ...
                                            <div key={item.id} className={`group flex items-center justify-between p-2 rounded-xl bg-panel-base border transition-colors shadow-sm ${isSoldOut ? "border-transparent opacity-50" : "border-panel-inactive hover:border-gray-600"}`}>
                                                <div className="flex flex-col min-w-0 flex-1 pr-2">
                                                    <div className="flex items-center gap-1.5 truncate">
                                                        {isGM && !item.isVisible && <span className="text-[13px] bg-[#9c81d8] text-mirage-400 px-1 py-0.5 rounded font-black shrink-0">隱藏</span>}
                                                        <span className={`font-bold text-[18px] truncate ${RARITY_COLORS[item.rarity || 'none']}`}>{item.name}</span>
                                                        {isSoldOut && <span className="text-[11px] bg-red-900/50 text-red-400 px-1 py-0.5 rounded font-black shadow-sm shrink-0">售罄</span>}
                                                    </div>
                                                    <div className="flex items-center gap-3 mt-1 text-[13px] text-gray-400 font-bold">
                                                        <span className="text-amber-400">{finalPrice.toFixed(2)} GP</span><span>•</span><span>{item.weight} lb</span><span>•</span><span>庫存: {item.quantity}</span>
                                                    </div>
                                                ...
                                                {isGM ? (
                                                    <div className="flex flex-col gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity shrink-0 ml-2">
                                                        ...
                                                        <div className="flex items-center justify-between w-[84px] bg-panel-content rounded-full p-0.5 shadow-sm border border-panel-inactive self-end">
                                                            <button onClick={() => saveShops(shops.map(s => s.id === shop.id ? { ...s, items: s.items.map(i => i.id === item.id ? { ...i, isVisible: !i.isVisible } : i) } : s))} className={`w-6 h-6 outline-none flex justify-center items-center ${item.isVisible ? "text-panel-active" : "text-gray-500 hover:text-white"}`}>{item.isVisible ? <FaEye className="w-3.5 h-3.5" /> : <FaEyeSlash className="w-3.5 h-3.5" />}</button>
                                                            <button onClick={() => { setEditingListType("items"); setTargetShopId(shop.id); setEditingItem(item); setFormItem(item); setItemModalOpen(true); playClickSound(); }} className="w-6 h-6 text-gray-400 hover:text-white outline-none flex justify-center items-center"><FaPencil className="w-3.5 h-3.5" /></button>
                                                    ...
                                                    <div className="flex flex-col items-end gap-2 shrink-0 ml-2 pt-1 border-l border-panel-inactive pl-3">
                                                        {!isSoldOut && (
                                                            <div className="flex items-center justify-between w-20 bg-panel-inactive/80 rounded-lg border border-panel-inactive overflow-hidden h-7">
                                                                <button className="flex-1 flex justify-center items-center font-bold text-gray-400 hover:text-white outline-none" onClick={() => setQty(item.id, buyQty - 1, item.quantity)}>-</button>
                                                        ...
                                                        <button disabled={isSoldOut || processingItems[item.id]} onClick={() => handleBuy(shop, item, buyQty, false)} className={`py-1.5 w-20 text-center rounded-lg text-xs font-bold outline-none transition-colors shrink-0 ${isSoldOut ? "bg-panel-inactive text-gray-500 cursor-not-allowed" : processingItems[item.id] ? "bg-gray-600 text-gray-400 cursor-wait" : "bg-panel-active text-white shadow-sm hover:opacity-80"}`}>
                                                            {processingItems[item.id] ? "處理中" : "購買"}
                                                        </button>
                                                    </div>
                                                )}
                                            </div>
                                    ...
                                    {shop.boughtItems && shop.boughtItems.length > 0 && shop.boughtItems.filter(i => isGM || i.isVisible).length > 0 && (
                                        <>
                                            <div className="mt-4 mb-1 px-1 flex justify-between items-end border-b border-panel-inactive/30 pb-1">
                                                <span className="text-xs font-bold text-amber-500">二手貨</span>
                                            </div>
                                            {shop.boughtItems.filter(i => isGM || i.isVisible).map(item => {
                                                ...
                                                return (
                                                    <div key={item.id} className={`group flex items-center justify-between p-2 rounded-xl bg-panel-base border transition-colors shadow-sm ${isSoldOut ? "border-transparent opacity-50" : "border-panel-inactive hover:border-gray-600"}`}>
                                                        <div className="flex flex-col min-w-0 flex-1 pr-2">
                                                            <div className="flex items-center gap-1.5 truncate">
                                                                {isGM && !item.isVisible && <span className="text-[13px] bg-mirage-800 text-mirage-400 px-1 py-0.5 rounded font-black shrink-0">隱藏</span>}
                                                                <span className={`font-bold text-[18px] truncate ${RARITY_COLORS[item.rarity || 'none']}`}>{item.name}</span>
                                                                {isSoldOut && <span className="text-[11px] bg-red-900/50 text-red-400 px-1 py-0.5 rounded font-black shadow-sm shrink-0">售罄</span>}
                                                            </div>
                                                            <div className="flex items-center gap-3 mt-1 text-[13px] text-gray-400 font-bold">
                                                        ...
                                                        {isGM ? (
                                                            <div className="flex flex-col gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity shrink-0 ml-2">
                                                                ...
                                                                <div className="flex items-center justify-between w-[84px] bg-panel-content rounded-full p-0.5 shadow-sm border border-panel-inactive self-end">
                                                                    ...
                                                                    <button onClick={() => { setEditingListType("boughtItems"); setTargetShopId(shop.id); setEditingItem(item); setFormItem(item); setItemModalOpen(true); playClickSound(); }} className="w-6 h-6 text-gray-400 hover:text-white outline-none flex justify-center items-center"><FaPencil className="w-3.5 h-3.5" /></button>
                                                            ...
                                                            <div className="flex flex-col items-end gap-2 shrink-0 ml-2 pt-1 border-l border-panel-inactive pl-3">
                                                                ...
                                                                <button disabled={isSoldOut || processingItems[item.id]} onClick={() => handleBuy(shop, item, buyQty, true)} className={`py-1.5 w-20 text-center rounded-lg text-xs font-bold outline-none transition-colors shrink-0 ${isSoldOut ? "bg-panel-inactive text-gray-500 cursor-not-allowed" : processingItems[item.id] ? "bg-gray-600 text-gray-400 cursor-wait" : "bg-panel-active text-white shadow-sm hover:opacity-80"}`}>
                                                                    {processingItems[item.id] ? "處理中" : "回購"}
                                                                </button>
                                                            </div>
                                                        )}
                                                    </div>
